...
class MainWindow:
    def __init__(self, master):
        ...
        self.login1 = ttk.Label(self.master, text='Введите логин')
        self.login1.pack(pady=10)
        self.login1 = ttk.Entry(self.master)
        self.login1.pack()
        self.password1 = ttk.Label(self.master, text='Введите пароль')
    ...
    def open_window2(self):
        ...
        columns = ("id","ip","date","Зона ","Строка запроса","код_состояния","размер объекта","ссылка","клиентский браузер","ОС "," webkit","версия webkit")
        ...
        def cat_vubor():
            ...
            rez = cur.execute(f'select * from LOGI where ip = "{self.kat}" and date="{self.kat1}" ').fetchall()
            columns = ("id", "ip", "date", "Зона ", "Строка запроса", "код_состояния", "размер объекта", "ссылка",
                       "клиентский браузер", "ОС ", " webkit", "версия webkit")
            self.tree.destroy()
            self.tree = ttk.Treeview(self.tab1Win2, bootstyle="success", columns=columns, show='headings')
            self.tree.pack(pady=10)
            for c in columns:
